...
import {
  createRowMappingWarningContext,
  findBrokenReferences,
  getRowMappingWarnings,
} from "../lib/reportLogic.js";
...
const isRowUnmapped = (row) =>
  ![row.dept, row.deptGroup, row.accCodes, row.groups].some((value) =>
    String(value || "").trim(),
  );
...
export default function RowsConfigurator({
  ...
  const [storedPresets, setStoredPresets] = usePersistentState(
  ...
  const rowWarningsById = React.useMemo(() => {
    const warningContext = createRowMappingWarningContext(
      activeReport.rows,
      masterData,
    );
    return new Map(
      ...
      activeReport.rows
        ...
        .filter(({ row }) => {
          if (rowFilter === "unmapped")
            return isDataRow(row) && isRowUnmapped(row);
          if (rowFilter === "warnings")
            return (
              isDataRow(row) && (rowWarningsById.get(row.id)?.length || 0) > 0
            );
          return true;
        }),
  ...
  const visibleSelectableIds = rowEntries
      ...
      <CardContent className="p-0">
        ...
        {brokenRowReferences.length > 0 && (
          <div className="border-b bg-destructive/5 px-4 py-2 text-sm text-destructive">
            Broken row references found. These {brokenRowReferences.length}{" "}
            invalid reference(s) may produce incomplete report data.
          </div>
        )}

        {undoBatch && (
          <section
            className="flex flex-wrap items-center justify-between gap-2 border-b bg-emerald-500/10 px-4 py-2"
            aria-live="polite"
          >
            <p className="text-sm tabular-nums text-foreground">
              Mapping updated for {undoBatch.count} rows.
            </p>
            <Button
              type="button"
              variant="outline"
        ...
        {bulkMode && (
          <section
            className="flex flex-wrap items-center gap-2 border-b bg-muted/20 px-3 py-2"
            aria-label="Bulk mapping toolbar"
          >
            <Filter
          ...
          <Table className="min-w-[1000px] table-fixed [&_td]:px-1 [&_th]:px-1">
                  ...
                  <TableHead className="w-12 text-center">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        toggleVisibleRows(checked === true)
                      }
                    />
                  </TableHead>
                )}
            ...
            <TableBody ref={containerRef}>
              {rowEntries.map(({ row, originalIndex: idx }) => {
                ...
                const rowReferenceIssues =
                  brokenReferencesByRowId.get(row.id) || [];
                const rowFormulaIssues = rowReferenceIssues.filter(
                  (issue) => issue.field === "formula",
                );
                const rowPercentBaseIssues = rowReferenceIssues.filter(
                  (issue) => issue.field === "percentBase",
                );

                const isPctBroken =
                  rowPercentBaseIssues.length > 0 ||
                  (row.percentBase && row.percentBase.includes("!REF!"));
                const isFormulaBroken =
                  rowFormulaIssues.length > 0 ||
                  (row.formula && row.formula.includes("!REF!"));
                  ...
                  >
                    ...
                    <TableCell className="px-2 py-2 align-middle">
                      <Input
                        value={row.desc}
                        onChange={(e) =>
                          handleUpdateRow(row.id, "desc", e.target.value)
                        }
                        className="min-w-0"
                      />
                    </TableCell>
                    <TableCell className="px-2 py-2 align-middle">
                      <Select
                        value={String(row.indent || 0)}
                        onValueChange={(value) =>
                          handleUpdateRow(row.id, "indent", parseInt(value, 10))
                        }
                      >
                    ...
                    <TableCell className="w-96 min-w-96 max-w-96 whitespace-normal px-2 py-2 align-middle">
                      {isHeader ? (
                        <span className="inline-flex rounded-full border border-border bg-background px-2.5 py-1 text-sm text-muted-foreground">
                          Header Row (No data mapping)
                        </span>
                      ) : isTotal ? (
                        <div
                          className={`flex items-center gap-2 rounded-lg border p-2.5 ${isFormulaBroken ? "border-destructive/60 bg-destructive/5" : ""}`}
                        >
                          <Calculator
                            className={
                              isFormulaBroken
                          ...
                          <Input
                            value={row.formula}
                            onChange={(e) =>
                              handleUpdateRow(row.id, "formula", e.target.value)
                            }
                            ...
                            <section className="min-w-0 space-y-1 pt-1">
                              {rowWarnings.map((warning) => {
                                ...
                                return isUnknown ? (
                                  ...
                                  <section
                                    key={warning}
                                    className="max-w-full break-words rounded-md border border-destructive/20 bg-destructive/5 px-2 py-1 text-xs text-destructive"
                                  >
                                    {warning}
                                  </section>
                                );
